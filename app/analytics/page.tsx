...
// Sparkline series built from real metric history or uniform baseline.
function sparkFor(id: string, value: number, historySeries?: Record<string, number[]>): number[] {
  if (historySeries && historySeries[id] && historySeries[id].length >= 7) {
    return historySeries[id].slice(-7);
  }
  return Array.from({ length: 7 }, () => Math.max(0, value));
}

// Real posting activity bars per channel.
function barsFor(platformKey: string, platformSeries?: Record<string, number[]>): number[] {
  if (platformSeries && platformSeries[platformKey] && platformSeries[platformKey].length > 0) {
    return platformSeries[platformKey].slice(-12);
  }
...
function MetricCard({ tile, historySeries }: { tile: MetricTile; historySeries?: Record<string, number[]> }) {
  ...
  return (
    <div className="hoverable flex flex-col gap-2.5 rounded-lg-t border border-os-border bg-os-surface px-[18px] py-4">
      ...
      <div className="flex items-end justify-between gap-2">
        <Spark data={sparkFor(tile.id, tile.value, historySeries)} w={96} h={26} />
...
export default async function AnalyticsPage() {
  ...
  const [attio, wispr, stripe, emailUnread, subs, postDays] = await Promise.all([
    ...
    zernioPostDays().catch(() => []),
  ]);

  const platformKeys: SocialPlatform[] = ['instagram', 'tiktok', 'twitter', 'youtube', 'linkedin'];
  const postSeries = postSeriesFromDays(postDays, today, 12, platformKeys);
  const platformSeries: Record<string, number[]> = {};
  for (const s of postSeries) {
    platformSeries[s.key] = s.points.map((p) => p.count);
  }
    ...
    <div>
      ...
      {live.length > 0 && (
        <section className="mb-6 grid gap-3.5 sm:grid-cols-2 xl:grid-cols-4 ultra:grid-cols-6">
          {live.map((tile) => (
            <MetricCard key={tile.id} tile={tile} historySeries={{ 'agent-runs': runVolume.map((v) => v.count) }} />
          ))}
        </section>
      )}
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex items-end justify-between gap-3">
                  ...
                  <MiniBars bars={barsFor(p.platform, platformSeries)} />
                </div>
