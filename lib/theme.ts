 ...
 * app/globals.css.
 */
export const THEMES = ['asteria-dark', 'asteria-midnight', 'mono', 'mono-light', 'dark', 'light', 'midnight', 'ember'] as const;
export type Theme = (typeof THEMES)[number];

/** What every fresh load gets until the user picks something else. */
export const DEFAULT_THEME: Theme = 'asteria-dark';
...
export const THEME_META: Record<Theme, { name: string; blurb: string; swatch: [string, string, string] }> = {
  'asteria-dark': { name: 'Asteria Dark', blurb: 'deep cosmos with cyan glow', swatch: ['#070a12', '#06b6d4', '#f8fafc'] },
  'asteria-midnight': { name: 'Asteria Midnight', blurb: 'midnight obsidian & violet starfire', swatch: ['#05050a', '#8b5cf6', '#f1f5f9'] },
